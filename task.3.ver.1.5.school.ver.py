...
def highestscore():
    print("this is a list of students sorted from highest to lowest score:")
    getcsv_class()
    highest_socres = []
    for row in csv_butts:
        highest_score,append(row)
        for row in sorted(highest_score,key=lambda x:(str(x[0]),int(x[1]),int(x[2])), reverse = True):
            print(row)
