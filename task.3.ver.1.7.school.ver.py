...
def highestscore():
    ...
    justfuckingwork = [high_score[x:x+4] for x in range(0,len(high_score),4)] #down to here it works
    print(justfuckingwork)
    for item in justfuckingwork[1:4]: #then here it gives the error "TypeError: 'function' object is not subscriptable" and like ???? wat
        print(max(justfuckingwork))
